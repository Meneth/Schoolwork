...
public class Bank {
	...
	private final long startTime; // Made this final
	private int nOfActiveCouriers; // Slightly renamed

	/**
	 * Creates a new bank, with a number of couriers, and starts all the
	 * couriers.
	 */
	public Bank(int nOfCouriers, int nOfVisits) {
		...
		for (int i = 0; i < nOfCouriers; i++) {
			...
			couriers[i].start(); // Starts the actual thread
		}
	}

	/**
	 * Deposit a given amount of money. The money is put in the night safe. If
	 * there is too much money in the night safe, all the money is transferred
	 * to the vault.
	 */
	public void depositMoney(int amount) {
		// Synchronized, or the overall amount could become inaccurate
	...
	public void courierDone() {
		// Synchronized, or it'd sometimes never reach 0
		synchronized (couriers) {
			nOfActiveCouriers--;
		}
		if (nOfActiveCouriers == 0) {
			...
			System.out
					.println("Discrepancy: " + (moneyDeposited - moneyInBank));
	...
	/**
	 * Sum up all the reported deposits to find out how much the couriers claim
	...
	/**
	 * Reads the number of couriers and the number of visits to the bank for
	 * each courier from the command line parameters, and starts the simulation.
	 */
	public static void main(String[] args) {
		if (args.length < 2) {
			System.out
					.println("Usage: java Bank <number of couriers> <number of visits>");
			System.exit(0);
		}
		int nOfCouriers = new Integer(args[0]).intValue();
		int nOfVisits = new Integer(args[1]).intValue();
		System.out.println("Starting simulation of " + nOfCouriers
				+ " couriers with " + nOfVisits + " deposits each.");
		new Bank(nOfCouriers, nOfVisits);
	}
}
