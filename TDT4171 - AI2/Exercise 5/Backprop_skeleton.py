...
def logFuncDerivative(x):
    res = math.exp(-x)/(pow(math.exp(-x)+1,2))
    return res
...
class NN: #Neural Network
    def __init__(self, numInputs, numHidden, learningRate):
        ...
        self.prevInputActivation = [1.0]*self.numInputs
        self.hiddenActivations = [1.0]*self.numHidden
        self.prevHiddenActivations = [1.0]*self.numHidden
        self.outputActivation = 1.0 #Assuming a single output.
        self.prevOutputActivation = 1.0 #Assuming a single output.
    ...
    def propagate(self, inputs, prev=False):
        if len(inputs) != self.numInputs-1:
            raise ValueError('wrong number of inputs')

        # input activations
        #self.prevInputActivations=listCopy(self.inputActivation)
        for i in range(self.numInputs-1):
            if prev:
                self.prevInputActivation[i] = inputs[i]
            else:
                self.inputActivation[i] = inputs[i]
        self.inputActivation[-1] = 1 #Set bias node to -1.
        self.prevInputActivation[-1] = 1  # Set bias node to -1.

        # hidden activations
        #self.prevHiddenActivations=listCopy(self.hiddenActivations)
        for j in range(self.numHidden):
            sum = 0.0
            for i in range(self.numInputs):
                #print self.ai[i] ," * " , self.wi[i][j]
                if prev:
                    sum = sum + self.prevInputActivation[i] * self.weightsInput[i][j]
                else:
                    sum = sum + self.inputActivation[i] * self.weightsInput[i][j]
            if prev:
                self.prevHiddenActivations[j] = logFunc(sum)
            else:
        ...
        for j in range(self.numHidden):
            if prev:
                sum = sum + self.prevHiddenActivations[j] * self.weightsOutput[j]
            else:
                sum = sum + self.hiddenActivations[j] * self.weightsOutput[j]
        if prev:
            self.prevOutputActivation = logFunc(sum)
        else:
            self.outputActivation = logFunc(sum)
        if prev:
            return self.prevOutputActivation
        else:
    ...
    def computeHiddenDelta(self):
        for i, out in enumerate(self.prevHiddenActivations):
            self.prevDeltaHidden[i] = logFuncDerivative(out) * self.weightsOutput[i] \
                                      * (self.prevDeltaOutput - self.deltaOutput)
        for i, out in enumerate(self.hiddenActivations):
            self.deltaHidden[i] = logFuncDerivative(out) * self.weightsOutput[i] \
                                  * (self.prevDeltaOutput - self.deltaOutput)

    def updateWeights(self):
        for i in range(self.numInputs):
            for j in range(self.numHidden):
                delta = (self.prevDeltaHidden[j] * self.prevHiddenActivations[j]) \
                        - (self.deltaHidden[j] * self.hiddenActivations[j])
                self.weightsInput[i][j] += self.learningRate * delta
    ...
    def train(self, patterns, iterations=1):
        ...
        for x in range(iterations):
            for pattern in patterns:
                self.propagate(pattern[0], True)
                self.propagate(pattern[1], False)
                self.backpropagate()
    ...
    def countMisorderedPairs(self, patterns):
        ...
        for pattern in patterns:
            self.propagate(pattern[0], True)
            self.propagate(pattern[1], False)
            if self.outputActivation > self.prevOutputActivation:
                numMisses += 1

        print(1 - (numMisses / len(patterns)), end='\t')