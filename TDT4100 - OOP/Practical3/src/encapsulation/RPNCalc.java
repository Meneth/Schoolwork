...
public class RPNCalc {
	...
	private int listSize; // Could simply use stack.size() directly, but this uses fewer cycles
	...
	public void performOperation(char operator) {
		...
		if (operator != '~') {
			push(Calculator.calculateResult(pop(), operator, secondOperand));
		} else {
	...
	public void computeUserInput() {
		...
		while (scanner.hasNext()) {
			if (scanner.hasNextDouble()) {
				...
				if (!Calculator.validOperator(operator)) { break; }
