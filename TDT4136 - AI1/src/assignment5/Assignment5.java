...
public class Assignment5 {
	...
	public static class CSP {
		public int backtracks, backtrackfails = 0;
		...
		public class Pair<T> {
			...
			public boolean equals(Object o) {
				Pair<T> comparison = (Pair<T>) o;
				return comparison.x.equals(x) && comparison.y.equals(y);
			}
		}
		 ...
		 */
		public ArrayDeque<Pair<String>> getAllArcs() {
			ArrayDeque<Pair<String>> arcs = new ArrayDeque<Pair<String>>();
		 ...
		 */
		public ArrayDeque<Pair<String>> getAllNeighboringArcs(String var) {
			ArrayDeque<Pair<String>> arcs = new ArrayDeque<Pair<String>>();
		...
		public VariablesToDomainsMapping backtrack(VariablesToDomainsMapping assignment) {
			backtracks++; // Keeping track of # of backtracks for assignment
			String unassignedValue = selectUnassignedVariable(assignment);
			if (unassignedValue == null)
				return assignment; // All variables have been assigned; solution found
			for ( String potentialValue : assignment.get(unassignedValue) ) {
				VariablesToDomainsMapping newAssignment = deepCopyAssignment(assignment);
				ArrayList<String> values = newAssignment.get(unassignedValue);
				values.clear();
				values.add(potentialValue); // Replace list with just the value
				if (inference(newAssignment, getAllNeighboringArcs(unassignedValue))) {
					newAssignment = backtrack(newAssignment);
					if (newAssignment != null)
						return newAssignment; // Successful solution
					else
						backtrackfails++; // Keeping track of # of backtrack failures for assignment
				}
			}
		...
		public String selectUnassignedVariable(VariablesToDomainsMapping assignment) {
			for (String key : assignment.keySet()) { // Picks the first unassigned value it finds
				if (assignment.get(key).size() != 1)
					return key;
			}
			return null; // No unassigned variable found
		}
		 ...
		 * @return 
		 */
		public boolean inference(VariablesToDomainsMapping assignment, ArrayDeque<Pair<String>> queue) {
			while (!queue.isEmpty()) {
				Pair<String> pair = queue.poll();
				if (revise(assignment, pair.x, pair.y)) {
					if (assignment.get(pair.x).size() == 0)
						return false; // The assignment is inconsistent
					for (String neighbor : constraints.get(pair.x).keySet())
						queue.add(new Pair<String>(pair.x, neighbor));
				}
			}
			return true;
		}
		...
		public boolean revise(VariablesToDomainsMapping assignment, String i, String j) {
			...
			for (String potentialValue : assignment.get(i)) {
				boolean pairingFound = false;
				for (String pairing : assignment.get(j)) {
					Pair<String> combination = new Pair<>(potentialValue, pairing);
					if (constraints.get(i).get(j).contains(combination)) {
						pairingFound = true;
						break;
					}
				}
				if (!pairingFound)
					valuesToRemove.add(potentialValue);
			}
			for (String value : valuesToRemove) {
				assignment.get(i).remove(value);
			}
			return valuesToRemove.size() != 0; // Whether any values have been removed as impossible
		}
	}
	...
	public static void main(String[] args) {
		...
		CSP sudoku = createSudokuCSP("sudokuBoards/easy.txt");
		printSudokuSolution(sudoku.backtrackingSearch());
		System.out.println("Backtracks: " + sudoku.backtracks);
		System.out.println("Backtrack fails: " + sudoku.backtrackfails);
		System.out.println("\nMedium board solution:");
		sudoku = createSudokuCSP("sudokuBoards/medium.txt");
		printSudokuSolution(sudoku.backtrackingSearch());
		System.out.println("Backtracks: " + sudoku.backtracks);
		System.out.println("Backtrack fails: " + sudoku.backtrackfails);
		System.out.println("\nHard board solution:");
		sudoku = createSudokuCSP("sudokuBoards/hard.txt");
		printSudokuSolution(sudoku.backtrackingSearch());
		System.out.println("Backtracks: " + sudoku.backtracks);
		System.out.println("Backtrack fails: " + sudoku.backtrackfails);
		System.out.println("\nVery hard board solution:");
		sudoku = createSudokuCSP("sudokuBoards/veryhard.txt");
		printSudokuSolution(sudoku.backtrackingSearch());
		System.out.println("Backtracks: " + sudoku.backtracks);
		System.out.println("Backtrack fails: " + sudoku.backtrackfails);
	}
